...
public class GraveLookupRequestCommand implements ICommand {
    ...
    public void processCommand(ICommandSender pCmdSender, String[] pArgs) {
        ...
        if (pArgs.length != 1) SendHelpToPlayer(tEP);
        else {
            if (pArgs[0].equalsIgnoreCase("request")) {
                String accessCode = UUID.randomUUID().toString().substring(26);

                try {
                    PreparedStatement ps = AMDIForge.instance.getConnection().prepareStatement(
                            "INSERT INTO `mcdata`.`lookupcodes` (`code`, `targetUID`, `targetName`) VALUES (?,?,?);");
                    ...
                    PlayerChatHelper
                            .SendInfo(tEP, String.format("Your Code is: %s It is valid for 10 Minutes", accessCode));
                } catch (SQLException e1) {
                    e1.printStackTrace();
                    PlayerChatHelper.SendError(
                            tEP,
                            "We're sorry, but something bad has happened on our side. Please contact a Staff Member, thank you!");
                }
            } else SendHelpToPlayer(tEP);
        }
    }
