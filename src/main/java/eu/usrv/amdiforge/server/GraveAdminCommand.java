...
public class GraveAdminCommand implements ICommand {
    ...
    public void processCommand(ICommandSender pCmdSender, String[] pArgs) {
        ...
        if (pArgs.length != 2) SendHelpToPlayer(tEP);
        else {
            String tSubCommand = pArgs[0];
            String tGraveArg = pArgs[1];
            File tFullGravePath = tEP.getEntityWorld().getSaveHandler()
                    .getMapFileFromName(PREFIX + stripFilename(tGraveArg));
            if (!tFullGravePath.exists()) {
                ...
                if (tSubCommand.equalsIgnoreCase("info") || tSubCommand.equalsIgnoreCase("i")) {
                    ...
                    AMDIForge.NW.sendTo(
                            new GraveContentDisplayMessage(GraveNBT.loadGraveFile(tFullGravePath.getAbsoluteFile())),
                            (EntityPlayerMP) tEP);
                    tEP.openGui(
                            AMDIForge.instance,
                    ...
                    if (tFullGravePath.delete()) PlayerChatHelper.SendNotifyPositive(tEP, "GraveFile has been deleted");
                    else PlayerChatHelper
                            .SendNotifyWarning(tEP, "GraveFile could not be deleted, something went wrong!");
    ...
    private void exportGraveToChest(EntityPlayer pEP, String pFullGravePath, boolean pForce) {
        try {
            ...
            if (tGrave.getPlacedFlag() != 0 && !pForce) {
                ...
                List<EntityPlayerMP> tAllPlayers = MinecraftServer.getServer()
                        .getConfigurationManager().playerEntityList;
                for (EntityPlayerMP tPlayer : tAllPlayers) {
                    if (tPlayer.canCommandSenderUseCommand(1, "")) {
                        PlayerChatHelper
                                .SendWarn(tPlayer, "%s just spawned a grave that was reported", pEP.getDisplayName());
            ...
            if (tTargetChest != null) {
                ...
                if (tExistingBlock != Blocks.air) {
                    PlayerChatHelper.SendError(
                            pEP,
                            "Unable to find some free space at your current location. Please move around and try again");
                    return;
                }
                ...
                if (tBlockChest != null) {
                    ...
                    if (entityChestInventory != null) {
                        ...
                        PlayerChatHelper
                                .SendNotifyPositive(pEP, "Grave-Export complete; ChestLocation: %d / %d / %d", x, y, z);
                    } else {
                        PlayerChatHelper
                                .SendError(pEP, "Unable to fill grave-contents to chest; chest export not possible");
                    }
                } else {
                    PlayerChatHelper
                            .SendError(pEP, "Unable to find IronChest in the GameRegistry, chest export not possible");
                }
            } else {
                PlayerChatHelper
                        .SendError(pEP, "Unable to find IronChest in the GameRegistry, chest export not possible");
    ...
    private void teleportToGrave(EntityPlayer pEP, String pFullGravePath) {
        GraveNBT tGrave = GraveNBT.getGrave(pFullGravePath);
        PlayerChatHelper.SendNotifyPositive(
                pEP,
                "Teleporting to GraveLocation at %d / %d / %d",
                (int) tGrave.getGraveLocation().xCoord,
                (int) tGrave.getGraveLocation().yCoord,
                (int) tGrave.getGraveLocation().zCoord);
        pEP.setPositionAndUpdate(
                tGrave.getGraveLocation().xCoord,
                tGrave.getGraveLocation().yCoord,
                tGrave.getGraveLocation().zCoord);
    }
    ...
    private void displayGraveInfo(EntityPlayer pCmdSender, String pGraveFile, String pFullGravePath) {
        try {
            ...
            tMultilineString = String.format(
                    tMultilineString,
                    pGraveFile,
                    tGrave.getCreated().toString(),
                    (tGrave.getPlacedFlag() == 1 ? EnumChatFormatting.DARK_GREEN + "Yes" + EnumChatFormatting.RESET
                            : EnumChatFormatting.DARK_RED + "No" + EnumChatFormatting.RESET),
                    (int) tGrave.getGraveLocation().xCoord,
                    (int) tGrave.getGraveLocation().yCoord,
                    (int) tGrave.getGraveLocation().zCoord,
            ...
            tGraveTeleport.HoverEvent = JSONHoverEvent
                    .SimpleText("Teleports you directly to the recorded spawn-location of this grave");
            ...
            tGraveExport.HoverEvent = JSONHoverEvent
                    .SimpleText("Exports the grave-contents to a chest spawned at your location");
    ...
    private void populateHelp() {
        ...
        jsonHelpGI.HoverEvent = JSONHoverEvent
                .SimpleText("Opens the Grave-Inspector GUI\nShows the content of the grave");
