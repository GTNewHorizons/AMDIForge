...
public class LivingCheckSpawnEventHandler {
    ...
    private boolean SaveSpawnLimits() {
        try {
            ...
            _mLogger.error(
                    "[SpawnLimits] Unable to create new MobSpawnLimits.xml. Is the config directory write protected?");
            e.printStackTrace();
            return false;
        }
    }
    ...
    public boolean reload() {
        ...
        if (_mInitialized) _mLogger.error("[SpawnLimits] Reload of SpawnLimits file failed");
    ...
    public void checkSpawnEvent(CheckSpawn pEvent) {
        ...
        if (!(pEvent.entityLiving instanceof EntityLiving)) return;

        if (pEvent.getResult() == Result.DENY) return; // BAIL! Someone else already said to deny that shit, so don't
                                                       // allow it
    ...
    private Event.Result getEventResult(World pWorld, Entity pEntity, Event.Result pCurrentEventResult) {
        ...
        if (tWorldList == null) {
            ...
            if (AMDIForge.AMDICfg.TraceAndExportUnknownEntities) {
                _mLogger.info(
                        String.format("Unknown Entity spawn detected: [%s] adding to config file", tEntityClassName));
        ...
        // No global whitelist. Check dimensions defined
            ...
            if (tTargetWorld != null) {
                ...
                if (tTargetWorld.getWhitelisted()) {
                    ...
                    int tActiveEntities = EntityCounter.getInstance()
                            .getEntityCountForWorld(tEntityClassName, tTargetWorld.getDimID());
        ...
        if (tReturn != Result.DEFAULT && AMDIForge.AMDICfg.DoDebugMessages) {
            _mLogger.info(
                    String.format(
                            "[SPAWNLIMITER] Attempt to spawn an instance of [%s] in World [%d]",
                            tEntityClassName,
                            tDimensionID));
            _mLogger.info(String.format(tDebugResultTemplate, tReturn.toString(), tResult));
        }

        // Now returns the previous Eventstate for this entity, instead of DEFAULT, which re-allowed all entities
        // blocked by Magnumtorch
