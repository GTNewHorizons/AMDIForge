...
@Mod(
        modid = AMDIForge.MODID,
        name = AMDIForge.MODNAME,
        version = AMDIForge.VERSION,
        dependencies = "required-after:Forge@[10.13.4.1558,);required-after:YAMCore@[0.5.69,);",
...
public class AMDIForge {
    ...
    public Connection getConnection() {
        try {
            if (!mSQL.checkConnection() || mCon == null) mCon = mSQL.openConnection();
    ...
    public void PreInit(FMLPreInitializationEvent pEvent) {
        ...
        if (AMDICfg.UseMySQL) {
            mSQL = new MySQL(
                    AMDICfg.MySQL_Server,
                    "3306",
                    AMDICfg.MySQL_DB,
                    AMDICfg.MySQL_User,
                    AMDICfg.MySQL_Password);
            try {
                ...
                Logger.error(
                        String.format(
                                "No MYSQL server could be reached! Set UseMySQL to false or remove the AMDIForce jar file."));
    ...
    public void serverLoad(FMLServerStartingEvent pEvent) {
        if (AMDICfg.EnableGraveAdminCommand) pEvent.registerServerCommand(new GraveAdminCommand());
        if (AMDICfg.EnableGraveHelpCommand) pEvent.registerServerCommand(new GraveLookupRequestCommand());
